...
class Patterns(object):
	...
	def urls(self):
		url_patterns = patterns('',
			...
			url(r'^(?P<category>[\w-]+)/list/(?:(?P<page>\d+)/)?$', blog_views.BlogCategoryView.as_view(), name = 'view'),
			url(r'^(?P<category>[\w-]+)/detail/(?P<slug>[\w-]+)$', blog_views.BlogDetailView.as_view(), name = 'detail'),
