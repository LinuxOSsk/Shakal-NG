...
INSERT INTO auth_user(id, password, last_login, is_superuser, username, first_name, last_name, email, is_staff, is_active, date_joined, jabber, url, signature, display_mail, distribution, original_info, filtered_info, year, avatar, settings)
	SELECT * FROM
		...
		AS t1(id integer, password character varying(128), last_login timestamp with time zone, is_superuser boolean, username character varying(30), first_name character varying(30), last_name character varying(30), email character varying(254), is_staff boolean, is_active boolean, date_joined timestamp with time zone, jabber character varying(127), url character varying(255), signature character varying(255), display_mail boolean, distribution character varying(50), original_info text, filtered_info text, year smallint, avatar character varying(100), settings text);
...
INSERT INTO news_news(id, category_id, title, slug, original_short_text, filtered_short_text, original_long_text, filtered_long_text, created, updated, authors_name, source, source_url, approved, author_id)
	SELECT * FROM
